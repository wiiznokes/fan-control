...
mod test {
    ...
    pub fn set_default() -> Result<(), NvmlError> {
        ...
        for device_id in 0..nvml.device_count()? {
            ...
            for fan_id in 0..device.num_fans()? {
                device.set_default_fan_speed(fan_id).unwrap();
            }
        }
    ...
    pub fn d() -> Result<(), NvmlError> {
        ...
        device.set_fan_speed(0, 100).unwrap();
    ...
    pub fn c() -> Result<(), NvmlError> {
        ...
        for device_id in 0..nvml.device_count()? {
            ...
            for fan_id in 0..device.num_fans()? {
                // device.set_fan_speed(fan_id, 100).unwrap();
                let s = device.fan_speed(fan_id)?;
                dbg!(&s);
            }
        }
    ...
    pub fn b() -> Result<(), NvmlError> {
        ...
        let clock_info = device.clock_info(Clock::Memory);
        let mem_clock = clock_info?;
